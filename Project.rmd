...
```
This is not a very good visual here, since it is hard to interpret however it does seem clear that suicide has been steadily increasing across several countries as years go on.
```{r}
...
```
The age range that most suicides seem to occur is 35-54 years of age.
```{r}
ggplot(data.merged) + ggtitle("Suicide Number Per age range") + geom_boxplot(aes(x= data.merged$age, y=data.merged$suicides_no)) +
xlab("Age Range") + ylab("suicide #")
ggplot(data.merged) + ggtitle("Suicide Number Per age range") + geom_col(aes(x= data.merged$age, y=data.merged$suicides_no)) +
xlab("Age Range") + ylab("suicide #")

```
The larger the population size the higher the suicide number also there seems to be a spike at population equal to about 1e+07. My theory is this may correspond to a specific region/ country.
```{r}
...
ggplot(data.merged)  + ggtitle("Scatterplot of population vs suicide number") +
geom_area(aes(x=data.merged$population, y=data.merged$suicides_no)) + xlab("population") + ylab("suicide")
...
The country with the lowest record is Albania(the data may be off for this since it is 0), Cyprus, Grenada and Antigua and Barbuda, and Iceland seem to all have very low suicude rates.
```{r}
max(data$suicides_no)
min(data$suicides_no)
data[data$suicides_no == max(data$suicides_no), ]
data[data$suicides_no == min(data$suicides_no) & data$year == "2016" , ] # Countires that have the lowest suicide numbers in recent years(2016) also checked 2015
data[data$suicides_no == min(data$suicides_no) & data$year > "2013" , ]
...
The US has the highest suicide number recorded out of the two countries with the highest being 11767 in 2010.
```{r}
data.merged[data.merged$suicides_no == min(data.merged$suicides_no), ]
data.merged[data.merged$suicides_no == max(data.merged$suicides_no), ]
