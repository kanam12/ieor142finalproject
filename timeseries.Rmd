...
trainAR_ts <- train_ts %>%
  mutate(LastYear=c(0, head(suicides.100k.pop, -1))) %>%
  mutate(TwoYearsAgo = c(0, 0, head(suicides.100k.pop, -2)))
...
OSR2(pred.test, trainAR_ts$suicides.100k.pop, test_ts_final$suicides.100k.pop)

pred.test.rf <- predict(mod.rf, newdata = test_ts_final)
OSR2(pred.test.rf, trainAR_ts$suicides.100k.pop, test_ts_final$suicides.100k.pop)
