...
us_ts = read.csv("us_suicides_merged.csv")
str(us_ts)
```
...
####BUILDING MODELS FOR INDIVIDUAL COLUMNS:
```{r}
...
```{r}
trainRW_ts <- train_ts %>% mutate(LastYear = c(NA, head(suicides.100k.pop, -1)))
head(trainRW_ts)
...
BaselineR2(tail(trainRW_ts$LastYear, -1), 
           tail(trainRW_ts$suicides.100k.pop, -1),
...
trainAR_ts <- train_ts %>%
  mutate(LastYear=c(NA, head(suicides.100k.pop, -1))) %>%
  mutate(TwoYearsAgo = c(NA, NA, head(suicides.100k.pop, -2)))
...
mod.rf <- randomForest(suicides.100k.pop ~ LastYear + TwoYearsAgo + year, data = tail(trainAR_ts, -2))
ggplot(trainAR_ts, aes(x=year, y=suicides.100k.pop)) +
  geom_line() +
  geom_point() +
...
# Test-set prediction
pred.test <- tail(predict(mod2b, newdata = test_ts_final), -2)
OSR2(pred.test, trainAR_ts$suicides.100k.pop, tail(test_ts_final$suicides.100k.pop, -2))

pred.test.rf <- tail(predict(mod.rf, newdata = test_ts_final), -2)
OSR2(pred.test.rf, trainAR_ts$suicides.100k.pop, tail(test_ts_final$suicides.100k.pop, -2))
