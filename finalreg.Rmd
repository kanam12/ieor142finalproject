...
```{r}
suicide_rates <- read.csv("final/suicide_rates_no_na.csv")
head(suicide_rates)
```
...
```{r}
model_us_train <- rpart(suicides.100k.pop ~., data = train_us, method = "class", cp= 0.1)
```
...
predcart_us = predict(model_us_train, newdata=us_test.cart, type="class")

#cart.tab.us <- table(test_us$suicides.100k.pop, predcart_us)

RMSE(predcart_japan, test_japan$suicides.100k.pop)
```
...
```{r}
model_japan_train <- rpart(suicides.100k.pop ~., data = train_japan, method = "class", cp= 0.1)
```
...
predcart_japan = predict(model_japan_train, newdata=japan_test.cart, type="class")
